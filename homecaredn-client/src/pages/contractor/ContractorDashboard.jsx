...
function StatusBadge({ status, t }) {
  const map = {
    Pending: { text: t("contractordashboard.pending"), cls: "bg-yellow-100 text-yellow-700" },
    Approved: { text: t("contractordashboard.approved"), cls: "bg-green-100 text-green-700" },
    Rejected: { text: t("contractordashboard.rejected"), cls: "bg-red-100 text-red-700" },
  };
  const cfg = map[status] || { text: status, cls: "bg-gray-100 text-gray-700" };
  return <span className={`px-2 py-1 text-xs rounded-full ${cfg.cls}`}>{cfg.text}</span>;
}
 ...
 return (
    <div className="relative">
      <button
        ...
        title={t("contractordashboard.change_language")}
      >
        ...
        <i className={`fas fa-chevron-down text-xs ${open ? "rotate-180" : ""}`} />
      </button>
  ...
  return (
    <div className="relative">
      <button
        ...
        title={t("contractordashboard.notifications")}
      >
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-72 bg-white border rounded-xl shadow-lg p-2 z-20">
          <div className="text-sm text-gray-700 px-2 py-1">
            {t("contractordashboard.you_have_notifications", { count: total })}
          </div>
          <div className="max-h-64 overflow-auto">
            {[...Array(Math.max(total, 1))].slice(0, 5).map((_, i) => (
              <div key={i} className="px-2 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg">
                {t("contractordashboard.notification_item", { number: i + 1 })}
              </div>
            ))}
          </div>
        </div>
...
function AvatarMenu() {
  ...
  return (
    <div className="relative">
      <button
          ...
          title={t("contractordashboard.account")}
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-48 bg-white border rounded-xl shadow-lg overflow-hidden z-20">
          ...
          > <i class="fa-solid fa-right-from-bracket text-red-500"></i> {t("header.logout")}
          </button>
        </div>
      )}
    </div>
  ...
  const kpis = useMemo(
    () => [
      { label: t("contractordashboard.kpi.open_requests"), value: 8,  meta: t("contractordashboard.kpi_meta.open_requests"),       cls: "text-green-600" },
      { label: t("contractordashboard.kpi.applied"),       value: 14, meta: t("contractordashboard.kpi_meta.applied"),             cls: "text-gray-600" },
      { label: t("contractordashboard.kpi.won"),           value: 5,  meta: t("contractordashboard.kpi_meta.won"),                 cls: "text-green-600" },
      { label: t("contractordashboard.kpi.pending_payments"), value: 3, meta: t("contractordashboard.kpi_meta.pending_payments"), cls: "text-red-600" },
    ],
    [t]
  );

  const items = SEED_APPS;
  const totalNotifications = items.reduce((s, it) => s + (it.notifications || 0), 0);
    ...
    <div className="min-h-screen grid grid-cols-[260px_1fr] bg-gray-50">
      ...
      <div className="flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-3 p-4">
            <div className="flex-1 flex items-center gap-2 bg-gray-100 border border-gray-200 rounded-xl px-3 py-2 text-gray-500">
              ...
              <input
                ...
                placeholder={t("contractordashboard.search_placeholder")}
              />
            </div>
        ...
        <main className="p-6 space-y-6">
          ...
          {/* Latest Applications */}
          <section id="applications" className="bg-white border border-gray-200 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold">{t("contractordashboard.latest_applications")}</h3>
              <button
                onClick={() => navigate("/applications")}
                className="text-sm px-3 py-1.5 rounded-lg border border-emerald-500 text-emerald-600 hover:bg-emerald-50"
              >
                {t("contractordashboard.view_all")}
              </button>
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="text-xs text-gray-500">
                  <tr className="border-b">
                    <th className="py-2 px-3 text-left">{t("contractordashboard.id")}</th>
                    <th className="py-2 px-3 text-left">{t("contractordashboard.description")}</th>
                    <th className="py-2 px-3 text-left">{t("contractordashboard.estimate")}</th>
                    <th className="py-2 px-3 text-left">{t("contractordashboard.last_update")}</th>
                    <th className="py-2 px-3 text-left">{t("contractordashboard.notifications")}</th>
                    <th className="py-2 px-3 text-left">{t("contractordashboard.status")}</th>
                    <th className="py-2 px-3 text-left">{t("contractordashboard.action")}</th>
                  </tr>
                </thead>
                <tbody>
                  {SEED_APPS.map((app) => (
                    <tr key={app.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-3 px-3">{formatDate(app.createdAt, i18n.language)}</td>
                      <td className="py-3 px-3">
                        {app.notifications > 0 ? (
                          ...
                          <span className="px-2 py-0.5 rounded-full text-xs bg-gray-200 text-gray-1000">0</span>
                        )}
                      </td>
                      <td className="py-3 px-3">
