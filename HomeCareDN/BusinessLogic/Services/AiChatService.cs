...
namespace BusinessLogic.Services
{
    public class AiChatService : IAiChatService
    {
        ...
        public async Task<AiChatResponseDto> SendAsync(AiChatRequestDto request)
        {
            ...
            shortHistory.Add(new AiChatMessageDto
            {
                Role = "user",
        ...
        public async Task<IEnumerable<AiChatMessageDto>> GetHistoryAsync(string id)
        {
            var key = CacheKey(id);
            var raw = await _cache.GetStringAsync(key);
            if (string.IsNullOrEmpty(raw)) return Enumerable.Empty<AiChatMessageDto>();
            return JsonSerializer.Deserialize<List<AiChatMessageDto>>(raw) ?? new();
        }

        public Task ClearHistoryAsync(string sessionId) => _cache.RemoveAsync(CacheKey(sessionId));
        ...
        private string EnsureCookieId()
        {
            ...
            _http.HttpContext!.Response.Cookies.Append(CookieName, id, new CookieOptions
            {
                HttpOnly = true,
                Secure = true,
        ...
        private string? ReadCookieId()
        {
            ...
            return ctx.Request.Cookies.TryGetValue(CookieName, out var v) && !string.IsNullOrWhiteSpace(v)
                ? v : null;
        }
        public async Task<IEnumerable<AiChatMessageDto>> GetHistoryAsync()
        {
            var id = ReadCookieId();
            if (id is null) return Enumerable.Empty<AiChatMessageDto>();
        ...
        public async Task ClearHistoryAsync()
        {
            ...
            var ctx = _http.HttpContext!;
            ctx.Response.Cookies.Append(CookieName, string.Empty, new CookieOptions
            {
                HttpOnly = true,
                Secure = true,
                SameSite = SameSiteMode.None,
